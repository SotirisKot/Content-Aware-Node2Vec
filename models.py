...
class BaselineNode2Vec(nn.Module):
    def __init__(self, vocab_size, embedding_dim):
        super(BaselineNode2Vec, self).__init__()
    ...
    def get_words_embeds(self, phr, pos, neg):
        ...
        if torch.cuda.is_available():
            phr = torch.LongTensor(phr).cuda()
            pos = torch.LongTensor(pos).cuda()
            neg = torch.LongTensor(neg).cuda()
        else:
            phr = torch.LongTensor(phr)
            pos = torch.LongTensor(pos)
            neg = torch.LongTensor(neg)

        phr = self.u_embeddings(phr)
        pos = self.v_embeddings(pos)
        neg = neg.view(phr.shape[0], config.neg_samples)
        neg = self.v_embeddings(neg)
        return phr, pos, neg

    def forward(self, pos_u, pos_v, neg_v):
        embed_u, embed_v, neg_embed_v = self.get_words_embeds(pos_u, pos_v, neg_v)
